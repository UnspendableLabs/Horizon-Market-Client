...
- **Native example: `@kontor/sdk-native` 0.3.0-rc.7, and the 16 KB page-size
  workaround is gone.** 0.5.0 carried a `withKontorPageSize` config plugin
  because the package's JNI/JSI adapter — compiled from its own CMakeLists
  during our Gradle build — still linked at 4 KB, which Play rejects.
  KontorProtocol/Kontor#563 shipped in rc.7: the flag now lives in the
  package's own `android/build.gradle`, so the plugin and its `app.json` entry
  are deleted. Verified against an untouched rc.7 — `libkontor-sdk-native.so`
  links at 0x4000 on arm64-v8a and x86_64 (armeabi-v7a stays 4 KB; 32-bit ARM
  has no 16 KB pages), and every native library in the assembled APK is 16 KB
  aligned. `@kontor/sdk` stays on rc.6, still its newest published version.

- **Native example: `npm run android` is `expo run:android` again.** The
  `scripts/run-android.cjs` wrapper existed only to keep the launch off
  16 KB-page devices, which it refused outright. With the adapter aligned the
  app installs, resolves its activity and runs on a `ps16k` API 37 image
  (`PAGE_SIZE=16384`), so the wrapper now only stood between a developer and a
...
- **Native example: the Android build supports 16 KB memory pages.** Play
  rejects an upload whose native libraries do not. `@kontor/sdk-native`'s
  prebuilt Rust core is already aligned, but the JNI/JSI adapter beside it is
  compiled from the package's own CMakeLists during the Gradle build, and NDK
  r27 links at 4 KB unless told otherwise — React Native's Gradle plugin passes
  `-DANDROID_SUPPORT_FLEXIBLE_PAGE_SIZES=ON` to the app's CMake project only,
  never to a library's. A `withKontorPageSize` config plugin now adds it to that
  subproject. It has to be a plugin: `android/` is generated by `expo prebuild`
  and gitignored, and EAS installs the package fresh from npm, so neither an
  edit in the generated tree nor a `node_modules` patch would reach a cloud
  build. Temporary, pending the upstream fix in KontorProtocol/Kontor#563.

- **Native example: no more photo-library permission.** Play flagged the upload
  for `READ_MEDIA_IMAGES` — access to the user's whole photo library, which the
  app never reads. It arrived by manifest merge from `expo-screen-capture`,
  which declares it for its screenshot-*detection* API; we use only
  `usePreventScreenCapture`, a window flag needing no permission. Both it and
  `READ_MEDIA_VIDEO` are now in `android.blockedPermissions`. Avatar picking is
  unaffected: `expo-image-picker` goes through the Android photo picker, which
  grants per-image access without either permission.
...
- The three example apps (`apps/cli`, `apps/native`, `apps/web`) leave their scaffold versions (`0.1.0` / `0.0.1` / `0.0.1`) for `1.0.0` — the version `apps/native/app.json` already ships to TestFlight — and `apps/native` gains the `private` field the other two already had. All three are unpublished and the CLI's `--version` comes from this package, so nothing user-visible moves.
