...
- **CLI: `horizon balances` stops printing `0` for a source it couldn't read.** Same bug, same shape — every read was folded into an empty list on rejection. XCP / ZELD / KOR now print `—` when their read failed (KOR also when `getKontorHoldings()` reports `unavailable`), each failure is named under the table, and `--json` gains an `errors` object (null per source on success) so scripts can tell "holds none" from "couldn't look".
